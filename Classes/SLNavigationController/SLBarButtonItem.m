...
- (void)setOptions:(NSDictionary *)options {
    ...
    //*计算改变文字大小 更改的Frame
    self.frame = CGRectMake(0, 0, buttonSize.width, [[self valueForOptionskey:SLbuttonSizeH]floatValue]);
    ...
    //为了区分 按钮是否为文字图片按钮 以及 是否位置转换，使用default 记录状态 ，10 为  非系统默认 11 为系统默认
        if (self.deafult == 10) {
            ...
            [self setTitleEdgeInsets:UIEdgeInsetsMake(0, -(imageSizeW + interval) , 0, imageSizeW + interval )];
                 self.frame = CGRectMake(0, 0, buttonSize.width + imageSizeW + interval , [[self valueForOptionskey:SLbuttonSizeH] floatValue]);
